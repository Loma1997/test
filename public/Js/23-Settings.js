function ChangeContainer(index) {
    ...
    document.querySelectorAll(".ContainerStep")[index]
        .classList.add("active")

    document.querySelectorAll(".Wrapper-button button").forEach(item => {
        item.classList.remove("active")
    })

    document.querySelectorAll(".Wrapper-button button")[index]
        .classList.add("active")
}
...
function CompanyData() {
    ...
    let TypeCompany = document.querySelector(".TypeCompany")
    let CityCompany = document.querySelector(".CityCompany")
    let AddressCompany = document.querySelector(".AddressCompany")
    let PhoneCompany1 = document.querySelector(".PhoneCompany1")
    let PhoneCompany2 = document.querySelector(".PhoneCompany2")
    fetch("/CompanyDataSettings", {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            NameCompany: NameCompany.value, TypeCompany: TypeCompany.value,
            CityCompany: CityCompany.value, AddressCompany: AddressCompany.value,
            PhoneCompany1: PhoneCompany1.value, PhoneCompany2: PhoneCompany2.value,
        })
    })
        .then((res) => res.json())
        .then((Data) => { Toast(id = Data.id, txt = Data.txt,); })
...
function Settings() {
    ...
    let Notfications = document.querySelector("#Notfications")
    ...
    fetch("/Settings", {
        ...
        body: JSON.stringify({
            ...
            Notfications: Notfications.checked,
