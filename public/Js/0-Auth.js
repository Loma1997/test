...
function YourINFO(ID) {
  ...
  // Check Name
  if (Name.value.trim() == "") { Name.classList.add("Required"); return Toast(id = "Error", txt = "يرجي ادخال الاسم",); }
  ...
  fetch("/isCurrentEmail", {
    ...
    .then((res) => res.json())
    .then((Data) => {
      if (Data[0].id === "Success") { GoStepNext(ID) }
      else { Toast(id = Data[0].id, txt = Data[0].txt,); }
    })
}
